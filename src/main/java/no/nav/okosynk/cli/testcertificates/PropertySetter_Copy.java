...
public class PropertySetter_Copy {
    ...
    public final void setOn(
        ...
        String propName;
        for(Iterator i$ = this.propertiesToSet.stringPropertyNames().iterator(); i$.hasNext(); properties.setProperty(propName, this.propertiesToSet.getProperty(propName))) {
            propName = (String)i$.next();
            if (properties.containsKey(propName)) {
                LOG.warn("Overwriting {} = {} to {} = {}", new Object[]{propName, properties.getProperty(propName), propName, this.propertiesToSet.getProperty(propName)});
            } else {
