    ...
    try (CloseableHttpResponse response = this.httpClient.execute(request)) {
      ...
      if (statusLine.getStatusCode() == HTTP_OK) {
        ...
        if (isNotBlank(aktoerIdent.getFeilmelding())) {
          return AktoerRespons.feil(String
          ...
          if (identer != null && !identer.isEmpty()) {
            if (identer.size() == 1) {
              ...
              return AktoerRespons.feil(String
                  .format("Forventet kun en aktoerId i respnsen men fikk: %s", identer.size()));
            }
          } else {
            return AktoerRespons.feil(
        ...
        return AktoerRespons.feil(String
            .format("Feil ved kall mot Aktoerregister - %s %s", statusLine.getStatusCode(),
